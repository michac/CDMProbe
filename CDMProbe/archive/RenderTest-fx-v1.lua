-- ARCHIVED 2026-08-02 — THE `/cdmp rt fx` DIALLING RIG, retired from RenderTest.lua.
--
-- NOT LOADED.  Not in the .toc, never parsed by the game.  This is the rig that dialled
-- the v1 cue treatment (archive/cue-treatment-v1.lua) — every knob here reaches into the
-- renderer instance's `cueGlows` / `cueEchoes` / `cueLayers.pop` pools, and the v2 cue has
-- none of those, so it could not run today even if it were loaded.
--
-- WHY IT WENT WITH v1.  It was built to answer open questions about a treatment that no
-- longer exists: `bg` / `glow N` / `ring` / `spin` / `rays` / `desync` / `art` / `pop` /
-- `ghost` / `sound`.  Round one of those questions was answered and PROMOTED into
-- Renderer.lua; the treatment it promoted has now itself been replaced by an experimental
-- result rather than by dialling (see Renderer.lua's RING_SCALE header).  Keeping a
-- dialling rig for a retired look is how a rig starts lying — the `rt fx` alpha bug, where
-- the rig wrote 1.0 over the Renderer's 0.45 and so showed a ring at >2x shipped
-- brightness for six builds, is exactly that failure and it cost most of a week.
--
-- ⚠ IF A NEW DIALLING RIG IS EVER NEEDED, WRITE A NEW ONE against the v2 pools.  Do not
-- revive this: its value was never the knob plumbing, it was the two rules it taught, and
-- both are recorded where they apply — (1) a rig that claims to be a faithful A/B baseline
-- must be asserted against the shipped path, not assumed; (2) an effect that wins gets
-- PROMOTED into the Renderer properly (a token, a GLOW_SPEC field, a one-shot channel),
-- never shipped from the rig.
--
-- Recover the `sound` auditioner from here if the cue sound is ever re-picked — that part
-- is treatment-independent (it plays files, it does not touch the cue pools).
--------------------------------------------------------------------------------
