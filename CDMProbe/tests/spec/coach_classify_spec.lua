...
-- re-pointed at the State pulse, and lands the key redesign: a hard-CD with an
-- ELAPSED napkin is ROTATION-eligible (napkinProbablyUp), NOT floored to NEVER.
-- These tests drive it against real golden state.json entries and assert the
-- CANDIDATE RECORD (not a final level — that is the cascade's job, covered by
-- coach_golden_spec).
...
describe("Coach.Classify", function()
  ...
  it("marks an ELAPSED napkin as probably-up (the ROTATION-eligible redesign)", function()
    -- tyrant-ready: Tyrant cd is unknown/napkin (estimate elapsed) — probably-up.
    local rec = recFor("tyrant-ready", 265187)
    assert.is_truthy(rec)
    assert.is_true(rec.napkinProbablyUp)
    assert.is_false(rec.cooling)
    assert.is_false(rec.anticipated)
  end)

  it("marks a counting-down napkin as anticipated, not probably-up", function()
    -- soon-anticipated: Tyrant is anticipated with ~2s remaining.
    local rec = recFor("soon-anticipated", 265187)
    assert.is_true(rec.anticipated)
    assert.is_false(rec.napkinProbablyUp)
    assert.equals(2.0, rec.remaining)
  end)

  it("marks a far cooldown as cooling (not a candidate)", function()
    -- hand-of-guldan: Dreadstalkers cooling ~7.5s.
    local rec = recFor("hand-of-guldan", 104316)
    assert.is_true(rec.cooling)
    assert.is_false(rec.napkinProbablyUp)
  end)
  ...
  it("flags overdue only for an elapsed-past-the-lead probably-up press", function()
    ...
    assert.is_true(overdue.napkinProbablyUp)
    ...
    assert.is_true(fresh.napkinProbablyUp)
